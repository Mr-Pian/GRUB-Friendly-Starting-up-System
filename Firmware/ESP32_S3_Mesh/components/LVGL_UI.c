        ...
        if(!g_is_provisioning) {
            ...
            if(strstr(current_text, "Waiting")) {
                if(g_wifi_connected) {
                    ...
                    // 没连上（取消了），恢复初始白色文字
                    lv_label_set_text(lbl_wifi, LV_SYMBOL_WIFI " Setup Campus Network");
...
static void wifi_setup_cb(lv_event_t * e) {
    if(!g_is_provisioning) {
        ...
        if(lbl_wifi) {
            lv_label_set_text(lbl_wifi, LV_SYMBOL_WIFI " Waiting... (Press to Cancel)");
...
static void init_settings_screen() {
    ...
    lv_obj_set_style_text_color(lbl_bri, lv_color_hex(0xFFFFFFF), 0);
    lv_obj_align(lbl_bri, LV_ALIGN_TOP_LEFT, 165, 42);
    ...
    lv_obj_set_style_text_color(lbl_aod, lv_color_hex(0xFFFFFFF), 0);
    lv_obj_align(lbl_aod, LV_ALIGN_TOP_LEFT, 165, 72);
...
static void init_aod_screen() {
    ...
    lv_label_set_text(label_time, "1425");
